...
enum enBank { List = 1, New = 2, Delete = 3, Update = 4, Find = 5, Transactions = 6, Exit = 7 };
enBank ReadMainMenueOperatoins(string);
...
void AddNewClient()
{
	...
	AddRecordToFile(ClientsFile, ConvertRecordToLine(Client));
}
void AddNewClients()
{
	char Answer = 'y';
...
void DeleteRecordFromFile(string FileName, vector<stClient> VClientsData)
{
	fstream File;
	string ClientData = "";
	...
	if (File.is_open())
	{
		for (stClient& C : VClientsData)
		{
			...
			{
				ClientData = ConvertRecordToLine(C);
...
bool DeleteClientByAccountNumber(string FileName, string AccountNumber, vector<stClient>& VClientsData)
{
	...
	else
	{
		...
		if (toupper(Answer) == 'Y')
		{
			...
			DeleteRecordFromFile(FileName, VClientsData);
...
void SaveRecordToFile(string FileName, vector<stClient> VClientsData)
{
	fstream File;
	string ClientData = "";
	File.open(FileName, ios::out);
...
bool UpdateClientByAccountNumber(string AccountNumber, vector<stClient>& VClientsData)
{
	...
	else
	{
		...
		if (toupper(Answer) == 'Y')
		{
			...
			SaveRecordToFile(ClientsFile, VClientsData);
...
void BankProgram()
{
	system("cls");
	MainMenueScreen();
	PerformMainMenueOperatoins(ReadMainMenueOperatoins("Choose what do you want to do? [1 to 7]"));
}
